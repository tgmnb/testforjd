...
from engine.multi_v3 import run_backtest
...
<div class="sub">GPT策略定义 + tqsdk数据 · MA20/MA75 · 成交量选主力 · 换月强平 · 滑点0.2% · OHLC填充 · N-bar保护止损 · {len(trades)}条记录</div></div>
<div class="ct">
<div class="mg">{t1}</div>
...
def main():
    print("[REPORT] Running v3 backtest...")
    periods = discover_main_contracts("DCE.jd", 2022, 2026)
    contract_data = download_all_contracts(periods)
    result = run_backtest(periods=periods, contract_data=contract_data, initial_capital=INITIAL_CAPITAL, short_ma=20, long_ma=75)

    print("[REPORT] Computing metrics...")
    metrics = compute_advanced_metrics(result.equity_curve, result.equity_dates, result.trade_journal.records, INITIAL_CAPITAL)
    chart = generate_equity_chart(result.equity_curve, result.equity_dates, metrics["max_drawdown_start_idx"], metrics["max_drawdown_end_idx"])
    html = generate_html_report(metrics, result.trade_journal.records, chart, "v3")

    out = Path(__file__).resolve().parent / "results" / "report_v3.html"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(html, encoding="utf-8")
    print(f"[REPORT] -> {out}")
    print(f"[REPORT] {metrics['total_return_pct']:+.2f}% / {metrics['num_trades']} trades")
